    ...
    const fetchEvaluation = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.error || "No se encontró la evaluación");
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Manejar específicamente errores de autenticación
        if (response.status === 401) {
          console.error("Error de autenticación al guardar evaluación:", errorData);
          toast({
            title: "Error de autenticación",
            description: "Tu sesión ha expirado. Por favor, inicia sesión nuevamente.",
            variant: "destructive",
          ...
          // Redirigir a la página de login (no opcional)
          router.push('/login');
          return;
        }
